...
const postBillList = (req, callback) => {
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            var Sql = 'insert into bill_statue (bill_number, submission_time, table_number, total_price) VALUES (?,?,?,?);'
            connection.query(Sql, [bill_number, submission_time, table_number, money], function (err, result) {
                if (err) {
                    result = {
                        err: true,
                        result: "插入失败",
